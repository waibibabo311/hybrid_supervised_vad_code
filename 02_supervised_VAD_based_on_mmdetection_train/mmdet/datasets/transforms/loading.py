...
@TRANSFORMS.register_module()
class Loadprompt(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        file_path = results['img_path']
        # print("加载prompt")
        file_name_with_extension = os.path.basename(file_path)
        # # 使用 os.path.abspath 将路径转换为绝对路径，去除其中的 ../ 部分
        file_dir = os.path.abspath(os.path.dirname(file_path))

        # # 使用 os.path.dirname 获取 file_dir 的上一级目录
        parent_dir = os.path.abspath(os.path.join(file_dir, os.pardir))

        # # 使用 os.path.splitext 分离文件名和扩展名，得到一个元组 (file_name, extension)
        file_name, extension = os.path.splitext(file_name_with_extension)
        file_path_prompt = os.path.join(parent_dir, 'annotations/dense_region_prompt_4', file_name+'.txt')
        # file_path_prompt = file_path.replace('.jpg', '.txt')
        # # print(file_path_prompt)
        # with open(file_path_prompt, 'r') as file:
        #     prompt = file.read()
        ...
        with open(file_path_prompt, 'r') as file:
            ...
            # Remove '\n' from each sentence
            prompt = [sentence.strip() for sentence in prompt]
...
@TRANSFORMS.register_module()
class Load_dense_region(BaseTransform):
    """Load an image from file.
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        # 使用 os.path.splitext 分离文件名和扩展名，得到一个元组 (file_name, extension)
        file_name, extension = os.path.splitext(file_name_with_extension)
        # file_path_prompt = os.path.join(parent_dir, 'annotations/new_annotations_all1', file_name+'.txt')
        file_path_prompt = os.path.join(parent_dir, 'annotations/new_dense_4', file_name+'.txt')
        # print(file_path_prompt)
        # 打开文件并读取内容
        with open(file_path_prompt, 'r') as file:
            dense_region = file.readlines()
        
         # 提取每一行的前四个数字
        dense_region_bboxes = []
        for line in dense_region:
            line = line.strip().split(',')
            bbox = [int(num) for num in line[:4]]
            dense_region_bboxes.append(bbox)
        # print(dense_region_bboxes)
        results['dense_region_bboxes']=dense_region_bboxes
        # print(results)
        return  results
...
@TRANSFORMS.register_module()
class Load_difficult_object(BaseTransform):
    """Load an image from file.
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        # 使用 os.path.splitext 分离文件名和扩展名，得到一个元组 (file_name, extension)
        file_name, extension = os.path.splitext(file_name_with_extension)
        file_path_prompt = os.path.join(parent_dir, 'annotations/new2', file_name+'.txt')
        # print(file_path_prompt)
        # 打开文件并读取内容
        with open(file_path_prompt, 'r') as file:
            dense_region = file.readlines()
        ...
        def calculate_iou(box1, box2):
            ...
            计算两个边界框的交并比（IoU）
            参数：
            box1, box2：两个边界框，表示为 [x1, y1, x2, y2]
            返回：
            iou：交并比值
            """
            x1_box1, y1_box1, x2_box1, y2_box1 = box1[:4]
            x1_box2, y1_box2, x2_box2, y2_box2 = box2[:4]
            x2_box1=x1_box1+x2_box1
            y2_box1=y1_box1+y2_box1
            x2_box2=x1_box2+x2_box2
            y2_box2=y1_box2+y2_box2
            # 计算交叉区域的坐标
        ...
        # 提取每一行的前四个数字
        difficult_object_bboxes = []
        difficult_object_prompt=[]
        easy_detection_object=[]
        for line in dense_region:
            line = line.strip().split(',')
            if ((line[6]=='1' or line[7]=='2') and int(line[5])<=10 ):
                bbox = [int(num) for num in line[:4]]+[int(line[5])]
                # bbox = [int(num) for num in line[:4]]
                # print(bbox)
                difficult_object_bboxes.append(bbox)
                # print(difficult_object_bboxes)
                if line[6]=='1':
                    object_prompt=f"a truncated {categories[int(line[5])]}"
                    # print(object_prompt)
                    difficult_object_prompt.append(object_prompt)
                else:
                   object_prompt=f"a heavily obscured {categories[int(line[5])]}"
                #    print(object_prompt)
                   difficult_object_prompt.append(object_prompt)
                # print("difficult_object_prompt",difficult_object_prompt)
        for difficult_box in difficult_object_bboxes:        
            max_iou = 0
            max_iou_bbox = []
            # print(difficult_box)
            # print(difficult_box[0])
            for box_gt in dense_region:
                line = box_gt.strip().split(',')
                box_gt = [int(num) for num in line[:4]]+[int(line[5])]
                # print(box_gt)
                if box_gt in difficult_object_bboxes:
                    continue
                # print(difficult_box,box_gt)
                # print(int(label[i]+1))
                # print(int(difficult_box[4]))
                if (int(difficult_box[0])==int(box_gt[0]) and int(difficult_box[1])==int(box_gt[1])) or int(box_gt[4])!=int(difficult_box[4]):
                    continue
                iou = calculate_iou(box_gt, difficult_box)
                # print(iou)

                if iou > max_iou:
                    max_iou = iou
                    max_iou_bbox = box_gt                   
            easy_detection_object.append(max_iou_bbox)
        # print("困难目标:",difficult_object_bboxes)
        # print("易检测目标：",easy_detection_object)
        # 在图像上绘制困难目标
        # 定义保存路径
        # save_folder = "/userHome/yym/code/mmdetection-3.x/easy_object_visual"

        # image = cv2.imread(file_path)
        # for box, prompt in zip(difficult_object_bboxes, difficult_object_prompt):
        #     x1, y1, width, height, category = box
        #     x2, y2 = x1 + width, y1 + height
        #     color = (0, 0, 255)  # 红色
        #     cv2.rectangle(image, (x1, y1), (x2, y2), color, 2)

        # # 在图像上绘制易检测目标
        # for box in easy_detection_object:
        #     if box:
        #         x1, y1, width, height, _ = box  # 注意，easy_detection_object中的每个框可能没有类别信息，所以用占位符"_"
        #         x2, y2 = x1 + width, y1 + height
        #         color = (0, 255, 0)  # 绿色
        #         cv2.rectangle(image, (x1, y1), (x2, y2), color, 2)

        # # 提取原始图像文件名（假设 file_path 是完整路径）
        ...
        results['easy_object']=easy_detection_object
        results['object_prompt']=difficult_object_prompt
        results['difficult_object_bboxes']=difficult_object_bboxes
        # print(results['object_prompt'])   
        # print(results['difficult_object_bboxes'])
                # print(results)
        return  results
