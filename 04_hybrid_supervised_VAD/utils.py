...
def normalize_within_range(tensor, range=(-1, 1)):
    # 1. Flatten along the (B) dimension
    original_shape = tensor.size()
    flattened_tensor = tensor.reshape(original_shape[0], -1)

    # 2. Normalize within the specified range
    min_val, max_val = range
    normalized_tensor = min_val + (flattened_tensor - flattened_tensor.min(dim=1, keepdim=True)[0]) / (flattened_tensor.max(dim=1, keepdim=True)[0] - flattened_tensor.min(dim=1, keepdim=True)[0]) * (max_val - min_val)

    # 3. Reshape back to original shape
    normalized_tensor = normalized_tensor.reshape(original_shape)
