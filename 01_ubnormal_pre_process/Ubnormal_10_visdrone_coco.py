...
def test():
    ...
    images = []
    annotations = []
    set = os.listdir('/data/gzh/UBnormal_Origin_For_OD_20240227/annotations')
    annotations_path = '/data/gzh/UBnormal_Origin_For_OD_20240227/annotations'
    images_path = '/data/gzh/UBnormal_Origin_For_OD_20240227/images'
    ...
    for i in tqdm(set):
        ...
        name = i.replace(".txt", "")
        image = {}
        height, width = cv2.imread(images_path + "/" + name + ".png").shape[:2]
        file_name = name + ".jpg"
        ...
        with open(f'/data/gzh/UBnormal_Origin_For_OD_20240227/output/output.json', 'w') as json_file:
